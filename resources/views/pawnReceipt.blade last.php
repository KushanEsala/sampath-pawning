                                                ...
                                                <div class="row">
                                                    <div class="col">
                                                        <label for="amount">AMOUNT :
                                                            <input class="form-control form-control-lg" style="font-weight: bold;" type="text" placeholder="AMOUNT:" id="amount" name="amount" oninput="validateAmount()"  required>
                                                            <input class="form-control form-control" style="font-weight: bold;" type="hidden" id="displayValiledPeriod" name="InterestRate" placeholder="Display Amount" readonly>
                                                        <input class="form-control form-control" style="font-weight: bold;" type="hidden" id="displayAmount" placeholder="Display Amount" readonly>
                                                        <input class="form-control form-control-lg" style="font-weight: bold;" type="hidden" id="displayinterestRate" name="InterestRate" placeholder="Display Amount"  readonly>
                                                        </label>
                                                    </div>
                                                    <div class="col">
                                            ...
                                            <table class="table table-bordered">
                                                <thead class="thead-light">
                                                    <tr>
                                                        ...
                                                        <th style="width:4%; text-align: center;">Receipt Type</th>
                                                        <th style="width:4%; text-align: center;">Receipt No</th>
                                                        <th style="width:5%; text-align: center;">Receipt Date</th>
                                                        <th style="width:5%; text-align: center;">Amount</th>
                                                        <th style="width:6%; text-align: center;">Total Amount</th>
...
<script>
    $(document).ready(function() {
        // Event handler for when typing in the 'amount' field
        $('#amount').on('keyup', function() {
            var amount = $(this).val(); // Get the input value
    
            // Send the value using AJAX to the controller
            $.ajax({
                url: '{{ route("Pawn_total_Value_Check_interest") }}', // The URL to the controller
                method: 'POST',
                data: {
                ...
                success: function(response) {
                    ...
                    $('#displayValiledPeriod').val(response.ValiledPeriod);
    
                    // Dynamically create and populate the table with response data
                    var table = $('#resultTable');
                    table.empty(); // Clear previous table data
    
                    var row = '<tr><td>' + response.amount + '</td><td>' + response.interestRate + '</td><td>' + response.ValiledPeriod + '</td></tr>';
                    table.append(row);
                },
