...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .customer-phone { left: 0.5cm; top: 5cm; }
        ...
        .advance-amount { left: 12cm; top: 12cm; color: #008000;font-weight: bold; }
...
@foreach($pawnSumData as $sumData)
    ...
    <div class="field customer-phone">Tel: {{ $sumData['Customer_Phone'] }}</div>

    <div class="field ticket-number">{{ $sumData['Receipt_Number'] }}</div>
    <div class="field receipt-date">{{ $sumData['Receipt_Date'] }} {{ $currentTimeFormatted }}</div>

    <div class="field branch">{{ Auth::user()->Branch }}</div>
    <div class="field branch-contact">{{ $branchData->contact1 ?? '' }}</div>
    ...
    @php
        ...
        $interestFor30Days = number_format((($sumData['Amount'] / 100) * $rate3));
        $interestFor10Days = number_format((($sumData['Amount'] / 100) * $rate1));
    @endphp

    <div class="field expiry-date">{{ $dueDate }}</div>
    ...
    <div class="field advance-amount">Rs. {{ number_format($sumData['Amount'], 2) }}</div>
