...
<script>
$(document).ready(function () {
    // Initially hide the pawning amount input field
    $('#pawning_amouunt').hide();

    // Trigger on AddvalueButton click
    $('#AddvalueButton').on('click', function (e) {
        e.preventDefault(); // Prevent any default action for the button

        // Show the pawning amount input field after clicking Add
        $('#pawning_amouunt').show(); 
    });

    // Trigger on keyup or change of the pawning_amount field
    $('#pawning_amouunt').on('keyup change', function (e) {
        let search_receipt_no = $('#total_amount').val();
        let receipt_type = $('#receipt_type').val();
        let pawning_amount = $('#pawning_amouunt').val(); // Get the pawning amount value
        let amount = $('#amount').val(); // Get the pawning amount value
        

        if (search_receipt_no > 0 && pawning_amount > 0) {
            ...
            $.ajax({
                ...
                data: {
                    search_receipt_no: search_receipt_no,
                    receipt_type: receipt_type,
                    pawning_amount: pawning_amount, // Pass pawning amount to the server
                    amount:amount
                },
                success: function (res) {
                    $('#showValuetable').html(res); // Update the table
                    if (res.status == 'not_found') {
                        $('#showValuetable').html(
                            `
                            <div style="text-align:center;">
                    ...
                    // Show the AddvalueButton after the table is populated
                    $('#AddvalueButton').show();
                }
            });
        } else {
            $('#showValuetable').html(''); // Clear table if conditions are not met
            $('#AddvalueButton').hide(); // Hide the button if conditions are not met
        }
    });
});
</script>
